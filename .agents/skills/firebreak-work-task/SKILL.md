...
4. Work only inside that workspace checkout. Today this may still use task- or worktree-named helpers such as `./scripts/new-worktree.sh <name>`.
5. Use workspace-owned VM state, validation artifacts, and review artifacts.
6. Close the workspace with a real disposition when that line of work is done or parked.
