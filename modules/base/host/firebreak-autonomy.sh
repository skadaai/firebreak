...
run_review() {
  ...
  filtered_changed_paths_log=$review_dir/changed-paths.filtered.log
  : >"$filtered_changed_paths_log"
  while IFS= read -r changed_path; do
    [ -n "$changed_path" ] || continue
    case "$changed_path" in
      .codex|.codex/*|.claude|.claude/*|.direnv|.direnv/*|result|result/*|*.img|*.socket)
        continue
        ;;
    esac
    printf '%s\n' "$changed_path" >>"$filtered_changed_paths_log"
  ...
  done <"$filtered_changed_paths_log"
  ...
  cat >"$review_path" <<EOF
{
  ...
  "changed_paths_path": "$(json_escape "$filtered_changed_paths_log")",
