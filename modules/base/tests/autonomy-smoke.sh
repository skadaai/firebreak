...
shared_escape_output=$(firebreak_cmd session create --session-id shared-escape --branch "$shared_escape_branch" --owner smoke)
shared_escape_worktree=$(extract_json_field "$shared_escape_output" worktree_path)
shared_escape_root=$autonomy_tmp_dir/shared-escape-root
...
if [ "$shared_escape_status" -eq 0 ] || ! printf '%s\n' "$shared_escape_summary" | grep -q '"blocked_reason": "policy-write-scope"'; then
  ...
  echo "autonomy smoke shared-escape scenario did not stop correctly" >&2
  exit 1
fi
...
if ! [ -f "$shared_escape_plan_path" ] || ! [ -f "$shared_escape_audit_root/review/write-scope.log" ] || ! grep -q '^.codex' "$shared_escape_audit_root/review/write-scope.log"; then
  printf '%s\n' "$shared_escape_summary" >&2
  echo "autonomy smoke shared-escape scenario did not preserve write-scope evidence" >&2
  exit 1
fi
close_session shared-escape blocked
