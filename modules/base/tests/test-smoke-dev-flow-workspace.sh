...
workspace_cmd validate --workspace-id spec-005-main test-smoke-codex-version >"$workspace_tmp_dir/workspace-one.validate.log" 2>&1 &
validate_one_pid=$!
workspace_cmd validate --workspace-id spec-006-main test-smoke-codex-version >"$workspace_tmp_dir/workspace-two.validate.log" 2>&1 &
validate_two_pid=$!
...
if ! find "$workspace_one_root/validation/test-smoke-codex-version/runs" -name summary.json -print -quit | grep -q .; then
  echo "workspace smoke did not preserve validation evidence for workspace one" >&2
  exit 1
fi

if ! find "$workspace_two_root/validation/test-smoke-codex-version/runs" -name summary.json -print -quit | grep -q .; then
  echo "workspace smoke did not preserve validation evidence for workspace two" >&2
  exit 1
fi

if ! [ -f "$workspace_one_root/artifacts/latest-validation-test-smoke-codex-version.json" ] || ! [ -f "$workspace_two_root/artifacts/latest-validation-test-smoke-codex-version.json" ]; then
