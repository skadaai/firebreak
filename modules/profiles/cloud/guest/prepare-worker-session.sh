...
if ! mountpoint -q @COMMAND_OUTPUT_MOUNT@; then
  if ! mount -t virtiofs hostexecoutput @COMMAND_OUTPUT_MOUNT@; then
    echo "failed to mount command output share" >&2
    exit 1
  fi
fi
...
@CAT@ "$prompt_source" > @TOOL_PROMPT_FILE@
chmod 0644 @TOOL_PROMPT_FILE@
