...
@JOB_PACKAGE_BIN@ \
  ...
  --state-dir "$state_dir" \
  --initial-prompt "Inspect the repository and print a short architecture summary to standard output"
...
if ! grep -q 'Inspect the repository and print a short architecture summary to standard output' "$output_dir/stdout.log"; then
...
@JOB_PACKAGE_BIN@ \
  ...
  --state-dir "$state_dir" \
  --initial-prompt "Run the timeout validation fixture" \
  --runtime-seconds 1
timeout_status=$?
set -e
