...
Feature: Cloud guest execution profile
  ...
  Scenario: Run a one-shot agent command against a prepared workspace
    ...
    And the requested initial prompt is "Inspect the repository and summarize the current architecture in ARCHITECTURE.md"
    ...
    And the guest shall start a new non-interactive Codex session for that prompt
