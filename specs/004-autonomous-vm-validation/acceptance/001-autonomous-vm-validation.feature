...
Feature: Autonomous VM validation
  ...
  Scenario: A runnable host executes a named Firebreak VM suite
    Given a host that satisfies the "test-smoke-codex" suite capabilities
    When the autonomous operator runs the "test-smoke-codex" validation suite
  ...
  Scenario: A blocked host does not pretend to have run the suite
    Given a host that does not satisfy the "test-smoke-codex" suite capabilities
    When the autonomous operator runs the "test-smoke-codex" validation suite
