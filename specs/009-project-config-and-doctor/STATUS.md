...
- tool-specific selectors now override generic selectors for their matching local workloads
- `host` mode now resolves through one shared host root with stable per-tool subdirectories for Codex and Claude Code
...
- 2026-03-25: Unified local `host` mode on one shared host root with stable `codex` and `claude` subdirectories, and removed per-tool host-path variables from the public config surface.
