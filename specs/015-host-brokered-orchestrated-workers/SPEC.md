...
- Packaged node-cli worker-proxy commands shall support a shared mode switch so the same command name can either launch a sibling Firebreak worker or delegate to the original packaged in-VM CLI without recipe-specific wrapper forks.
