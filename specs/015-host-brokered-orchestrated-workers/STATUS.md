...
- 2026-04-01: Closed a missing local-mode contract gap for packaged worker proxies by deriving local upstream CLI installation from the declared Firebreak worker package, instead of requiring external recipe authors to restate npm package/bin metadata. This also clarified that `launch_mode` and `worker-mode` are distinct public knobs rather than aliases.
